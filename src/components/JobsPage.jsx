...
const JobsPage = ({ onViewJob, onLoadJob }) => {
  ...
  const filterJobs = () => {
    if (!searchTerm.trim()) {
      setFilteredJobs([...jobs].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)))
      return
    }

    const filter = searchTerm.toLowerCase()
    const filtered = jobs.filter(
      (job) => job.id.toLowerCase().includes(filter) || job.timestamp.toLowerCase().includes(filter),
    )

    setFilteredJobs([...filtered].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)))
  }
